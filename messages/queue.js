// v5 Adding pushMessageQFunc2 that takes in storage account and name so map can be used
// v4 Modified pushMessageFunc to target different storage and queue
// v3 Added pushMessageQFunc variables to pass storage and queue name
// v2 Added Promise pushMessageFunc

var azure = require('azure-storage');

function pushMessageQFunc2(queuedMessage, storage) {
    var storage_name = storage.account_name;
    var queue_name = storage.queue_name;
...
function pushMessageFunc(queuedMessage) {
    var queueSvc = azure.createQueueService(process.env['expensenotifybotd3giz3_STORAGE']);

    return new Promise((resolve, reject) => {
        queueSvc.createQueueIfNotExists('js-queue-items-for-processing', (err, result, response) => {
            if (!err) {
                // Add the message to the queue
                var queueMessageBuffer = new Buffer(JSON.stringify(queuedMessage)).toString('base64');
                queueSvc.createMessage('js-queue-items-for-processing', queueMessageBuffer, (err, result, response) => {
                    if (!err) {
                        // Message inserted
                        console.log('Your message (\'' + queuedMessage.payload + '\') has been added to a queue, and it will be sent back to you via a Function');
                        resolve(result);
                        //resolve('Your message (\'' + queuedMessage.text + '\') has been added to a queue, and it will be sent back to you via a Function');
                    } else {
                        // this should be a log for the dev, not a message to the user
...
exports.pushMessageQFunc2 = pushMessageQFunc2;
